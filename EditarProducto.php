...
if ($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    if ($nombre === '' || $precio === '' || $idmarca <= 0 || $idcategoria <= 0){
        ...
        try{
            $sql = "UPDATE productos SET NombreProducto = ?, Precio = ?, DescripcionProducto = ?, Imagen = ?, IDmarca = ?, IDcategoria = ? WHERE ID = ?";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([$nombre, $precio, $descripcion, $imagen, $idmarca, $idcategoria, $id]);
            header('Location: GestionProductos.php?message=' . urlencode('Producto actualizado correctamente'));
            exit;
        } catch (PDOException $e){
...
<head>
    ...
    <title>Editar Producto - PapuShop</title>
...
<body>
    ...
    <div class="form-container">
        ...
        <form method="post" action="EditarProducto.php?id=<?php echo $id; ?>" class="product-form">
            
            <div class="form-row">
                <div class="form-group">
                    <label for="nombre" class="form-label">Nombre del Producto *</label>
                    <input type="text" id="nombre" name="nombre" class="form-input" value="<?php echo htmlspecialchars($producto['NombreProducto']); ?>" placeholder="Ej: Sweater Azul" required>
                </div>

                <div class="form-group">
                    <label for="precio" class="form-label">Precio *</label>
                    <input type="number" id="precio" name="precio" step="0.01" class="form-input" value="<?php echo htmlspecialchars($producto['Precio']); ?>" placeholder="0.00" required>
                </div>
            </div>

            <div class="form-group">
                <label for="descripcion" class="form-label">Descripción</label>
                <textarea id="descripcion" name="descripcion" class="form-textarea" placeholder="Descripción del producto..."><?php echo htmlspecialchars($producto['DescripcionProducto']); ?></textarea>
            </div>
                ...
                <div class="form-group">
                    ...
                    <select id="idcategoria" name="idcategoria" class="form-select" required>
                        ...
                        <?php foreach ($categorias as $c): ?>
                            <option value="<?php echo $c['ID']; ?>" <?php if ($producto['IDcategoria'] == $c['ID']) echo 'selected'; ?>><?php echo htmlspecialchars($c['NombreCategoria']); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select id="idmarca" name="idmarca" class="form-select" required>
                        ...
                        <?php foreach ($marcas as $m): ?>
                            <option value="<?php echo $m['ID']; ?>" <?php if ($producto['IDmarca'] == $m['ID']) echo 'selected'; ?>><?php echo htmlspecialchars($m['NombreMarca']); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <select id="imagen" name="imagen" class="form-select">
                    ...
                    <?php foreach ($imagenes as $img): ?>
                        <option value="<?php echo $img; ?>" <?php if ($producto['Imagen'] == $img) echo 'selected'; ?>><?php echo basename($img); ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
